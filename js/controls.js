...
function initControls(globals){
    ...
    globals.textureRepeat = 1;

    $("#textureFile").on('change', function(e) {
        var file = e.target.files[0];
        if (file) {
            if (file.type.startsWith('image/')) {
                globals.model.loadTexture(file, function() {
    ...
    setSliderInput("#textureRepeat", globals.textureRepeat, 0.1, 10, 0.1, function(val){
        globals.textureRepeat = val;
        globals.model.setMeshMaterial();
    });

    function setColorMode(val){
        globals.previousColorMode = globals.colorMode;
