...
function initModel(globals){
    ...
    function loadTexture(file, callback) {
        ...
        reader.onload = function(e) {
            ...
            textureLoader.load(
                ...
                undefined,
                function(error) {
                    globals.warn("Error loading texture. Please try a different file.");
                }
            );
        };
        reader.onerror = function(error) {
            globals.warn("Error reading texture file.");
        };
    ...
    function setMeshMaterial() {
        ...
        if (globals.colorMode == "normal") {
            ...
            currentTexture.repeat.set(globals.textureRepeat, globals.textureRepeat);
            material = new THREE.MeshBasicMaterial({
                map: currentTexture,
                side: THREE.DoubleSide,
                polygonOffset: true,
            ...
            if (globals.colorMode === "customTexture" && !currentTexture) {
                globals.warn("Please select a texture file to use Custom Texture mode.");
            }
    ...
    function sync(){
        ...
        // 1. Calculate bounding box of the original flat pattern
        var bbox = new THREE.Box3();
        for (var i = 0; i < vertices.length; i++) {
            bbox.expandByPoint(vertices[i]);
        }
        var size = bbox.getSize(new THREE.Vector3());
        if (size.x === 0) size.x = 1; // Prevent division by zero
        if (size.z === 0) size.z = 1; // Prevent division by zero

        // 2. Populate UV array based on normalized position
        for (var i = 0; i < vertices.length; i++) {
            var original_vertex = vertices[i];
            // The FOLD format uses x and y for 2D, which map to x and z in the 3D view.
            // We use the original x and z for our UVs.
            uvs[2 * i]     = (original_vertex.x - bbox.min.x) / size.x;
            uvs[2 * i + 1] = (original_vertex.z - bbox.min.z) / size.z;
        }
